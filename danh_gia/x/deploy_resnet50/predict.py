...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ResNet50 CNN or ResNet50+SVM inference.")
    parser.add_argument("--config", default=None, help="Deploy config. Default: deploy_resnet50/config.yaml")
...
def extract_features(model: nn.Module, batch: torch.Tensor, device: torch.device) -> np.ndarray:
    extractor = ResNet50FeatureExtractor(model).to(device)
    extractor.eval()
