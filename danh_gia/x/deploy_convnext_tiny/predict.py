...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ConvNeXt-Tiny CNN or ConvNeXt-Tiny+SVM inference.")
    parser.add_argument("--config", default=None, help="Deploy config. Default: deploy_convnext_tiny/config.yaml")
    parser.add_argument("--mode", choices=["cnn", "svm"], default="cnn", help="cnn = classifier, svm = CNN features + SVM.")
...
def build_convnext_tiny_model(num_classes: int) -> nn.Module:
...
def load_cnn(checkpoint_path: Path, num_classes: int, device: torch.device) -> nn.Module:
    ...
    model = build_convnext_tiny_model(num_classes=num_classes)
...
def extract_features(model: nn.Module, batch: torch.Tensor, device: torch.device) -> np.ndarray:
    extractor = ConvNeXtTinyFeatureExtractor(model).to(device)
    extractor.eval()
...
def main() -> None:
    ...
    batch_size = args.batch_size or int(cfg.get("runtime", {}).get("batch_size", 4))
